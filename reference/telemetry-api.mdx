...
`fatal`, but read responses return canonical severity values.

Unknown severity or level values are rejected on ingest.

## Log response shape

Log responses keep `level` for compatibility and include canonical `severity`
for user-facing consumers.
