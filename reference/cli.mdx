...
| Command | Purpose |
| --- | --- |
| `logbrew login [--json] [--no-open]` | Start human browser login for the CLI. |
| `logbrew setup [--auto] [--yes] [--json]` | Plan SDK setup for the current project. |
| `logbrew setup --create-project [--json]` | Show project creation discovery help. Help-only in v0.1.17. |
...
| `logbrew projects [--json]` | Show project discovery help. Help-only in v0.1.17. |
| `logbrew project [--json]` | Alias for project discovery help. Help-only in v0.1.17. |
| `logbrew projects create <name> [--json]` | Show project creation discovery help. Help-only in v0.1.17. |
...
| `logbrew usage [--json]` | Show usage discovery help. Help-only in v0.1.17. |
| `logbrew account usage [--json]` | Show account usage discovery help. Help-only in v0.1.17. |
...
`logbrew login` starts human browser login for the native CLI. The browser
returns to the CLI through a local loopback callback, then the user can verify
...
Use `--no-open` when the CLI cannot open a browser automatically, such as on a
remote shell. It prints the auth URL and waits for the user to open it manually.

```bash
logbrew login --no-open
```

Use `logbrew login --json` for a token-safe manual browser handoff. It does not
open a browser, but it does start login and print a first JSON event with
`stage: "auth_url"`, `auth_url`, `browser_opened: false`, `redirect_uri`, and
`next: "open auth_url in a browser to complete login"`. After the browser
callback completes and the CLI stores local auth, it prints a second JSON event
with `stage: "complete"`, `auth: "local_token"`, and
`next: "run logbrew status"`. It must not print local token material.

Public CLI auth uses `GET /api/auth/cli/login` to start login and
`POST /api/auth/cli/token` to complete the CLI token exchange.
...
CLI v0.1.17 exposes project creation and account usage commands as discovery
