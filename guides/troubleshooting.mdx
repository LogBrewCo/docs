...
| Agent needs auth | `logbrew login --json` | Ask a human to run `logbrew login`; JSON login does not complete auth. |
...
<Steps>
  <Step title="Check access first">
    ...
    `logbrew login --json` is token-safe guidance for agents; it does not open
    a browser, complete auth, or print local token material.
  </Step>
