...
| Browser cannot open | `logbrew login --no-open` | Open the printed auth URL manually. |
| Agent needs auth | `logbrew login --json` | Open the returned `auth_url` in a browser and wait for the `complete` event. |
...
<Steps>
  <Step title="Check access first">
    ...
    Use `logbrew login --no-open` when the CLI cannot open a browser for you.
    `logbrew login --json` is a token-safe manual browser handoff for agents:
    it prints an `auth_url` event, waits for the browser callback, and prints a
