...
`logbrew login --json` is intentionally inert and token-safe for agents. It
does not open a browser, does not complete auth by itself, and does not print
local token material. Agents should ask a human to run `logbrew login`, then
continue with `logbrew status --json`.
...
- Put `--json` before or after the command, such as `logbrew --json status` or
  `logbrew status --json`.
- `logbrew login --json` is guidance only: it does not open a browser, complete
  auth, or print local token material.
