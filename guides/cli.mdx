...
Run the human browser flow first.
...
The browser returns to the native CLI through a local loopback callback. Use
`logbrew login --no-open` when the CLI cannot open a browser automatically; it
prints the auth URL and waits for you to open it manually.

`logbrew login --json` is a token-safe manual browser handoff for agents. It
does not open a browser. It prints an `auth_url` event, waits for the browser
callback, then prints a `complete` event after local auth is stored. Agents
should ask a human to open the `auth_url`, then continue with
`logbrew status --json`. CLI output must not print local token material.
...
- Put `--json` before or after the command, such as `logbrew --json status` or
  `logbrew status --json`.
- `logbrew login --json` prints token-safe JSON handoff events: first
  `stage: "auth_url"` with `auth_url`, then `stage: "complete"` after the
  browser callback and local auth persistence.
