...
- Treat `logbrew login --json` as a token-safe manual browser handoff. It
  prints an `auth_url` event, waits for the browser callback, then prints a
  `complete` event after local auth is stored. It must not print local token
  material.
