...
CLI v0.1.30 or newer can create a backend-owned project and store its one-time
ingest key without dashboard interaction. Keep the flow short and safe.
...
<Steps>
  ...
  <Step title="Create the project securely">
    ```bash
    ...
    Project creation changes account state. The CLI creates the destination
    file with owner-only permissions before it reports success and never
    prints the key or its path. Repeat an ambiguous attempt only with the exact
    same command.
  </Step>
...
<Steps>
  <Step title="Check access and project state">
    ...
    CLI v0.1.30 or newer reads and creates backend-owned projects;
