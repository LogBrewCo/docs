...
| Go | `go get github.com/LogBrewCo/sdk/go/logbrew@latest` |
| Rust | `cargo add logbrew` |
| Java / Spring | Add `co.logbrew:logbrew-sdk:0.1.6` from Maven Central. |
| .NET | `dotnet add package LogBrew --version 0.1.7` |
| ASP.NET Core | `dotnet add package LogBrew.AspNetCore --version 0.1.2` |
| C++ / CMake | Pin `cpp/logbrew-cpp/v0.2.3` with CMake `FetchContent`. |

### ASP.NET Core

CLI v0.1.55 or newer detects .NET 10 projects that use
`Microsoft.NET.Sdk.Web`. Run its non-mutating planner from the project root:

```bash
logbrew setup --json
```

The released plan installs the host integration:

```bash
dotnet add package LogBrew.AspNetCore --version 0.1.2
```

Set `LOGBREW_SERVER_API_KEY`, `LOGBREW_SERVICE_NAME`,
`LOGBREW_ENVIRONMENT`, and `LOGBREW_RELEASE` outside source control. Register
LogBrew with the host and place its middleware after routing:
...
Without `LOGBREW_SERVER_API_KEY`, both registration calls are safe no-ops. See
the [ASP.NET Core package guide](https://github.com/LogBrewCo/sdk/tree/main/dotnet/logbrew-dotnet/src/LogBrew.AspNetCore)
for host lifecycle, request and issue capture, privacy limits, typed context,
dependency spans, hosted verification, and recovery.
